<?php 
$all_tickets_old = $args->event->all_tickets_data;
$all_tickets = EventM_Combined_Count_FrontEnd::ticket_template_data( $all_tickets_old );

print "<!-- 5 ".print_r($all_tickets, TRUE)."-->";

if ($all_tickets[0]->capacity != $all_tickets[0]->orig_capacity) {
    $epcc_capacity = true;
    $capacity_text = __('Overall Venue Capacity');    
}
else {
    $epcc_capacity = false;    
    $capacity_text = __('Capacity');    
}
...
// we'll want to roll in JS and knock down max 
...
<?php if( ! empty( $booking_allowed ) && $args->event->post_status == 'publish' && ! $is_event_expired ) {
    if( ! empty( $all_tickets ) && count( $all_tickets ) > 0 ) {
        ...
        $max_adjust = 0;
        if ($epcc_capacity) {
            foreach ($all_event_bookings['tickets'] as $key => $ticket_count) {
                $max_adjust += $ticket_count;                
            }
        }
                                                ...
                                                <div class="ep-box-col-12"><?php 
                                                    ...
                                                    if( $check_ticket_available['status'] == 'not_started' ) {?>
                                                        ...
                                                        if( ! empty( $booked_tickets_data ) ) {                                                        
                                                            if ($epcc_capacity) {
                                                                // the new way
                                                                $booked_ticket_qty = absint( array_sum($booked_tickets_data) );
                                                            ...
                                                            <div class="ep-btn-group btn-group-sm ep-mb-2 ep-select-event-ticket-number <?php echo esc_attr( $cursor_class );?>" role="group">
                                                                ...
                                                                if( ! empty( $ticket_disabled ) ) {?>
                                                                    ...
                                                                    <button type="button" class="ep-btn ep-btn-outline-dark ep-px-3 ticket_minus" data-parent_id="<?php echo absint( $ticket->id );?>"> - </button>
                                                                        <input type="number" name="em_ticket_qty_<?php echo absint( $ticket->id );?>" id="ep_event_ticket_qty_<?php echo absint( $ticket->id );?>" class="ep-btn ep-btn-outline-dark ep-px-3" min="0" max="<?php echo absint( $max_caps );?>" value="0" data-min_allowed="<?php echo absint( $ticket->min_ticket_no );?>" readonly data-epcc="<?php echo absint( $ticket->epcc_capacity - $max_adjust );?>">
